...
if ( is_admin() && ( !defined('DOING_AJAX') || !DOING_AJAX ) && !class_exists( 'GA_Admin' ) ) {
	...
	class GA_Admin extends Escalate_GA_Plugin_Admin {

		var $hook 		= 'wordpress-analytics';
		var $filename	= 'wordpress-analytics/wordpressanalytics.php';
		var $longname	= 'Google Analytics Configuration';
		var $shortname	= 'Google Analytics';
		var $ozhicon	= 'images/chart_curve.png';
		var $optionname = 'Escalate_Google_Analytics';
		var $homepage	= 'http://www.escalateseo.comwordpress/google-analytics/';
		var $toc		= '';
		...
		function plugin_help($contextual_help, $screen_id, $screen) {
			if ( $screen_id == 'settings_page_'.$this->hook ) {
				...
				'<p>'.sprintf( __("If you're having problems with this plugin, please refer to its <a href='%s'>FAQ page</a>."), 'http://www.escalateseo.comwordpress/google-analytics/ga-wp-faq/' ).'</p>';
			}
		...
		function save_settings() {
			...
			if ( isset($_REQUEST['reset']) && $_REQUEST['reset'] == "true" && isset($_REQUEST['plugin']) && $_REQUEST['plugin'] == 'wordpress-analytics') {
				$options = $this->set_defaults();
				$options['msg'] = "<div class=\"updated\"><p>".__('Google Analytics settings reset.')."</p></div>\n";
			} elseif ( isset($_POST['submit']) && isset($_POST['plugin']) && $_POST['plugin'] == 'wordpress-analytics') {
				if (!current_user_can('manage_options')) die(__('You cannot edit the WordPress Analytics options.'));
		...
		function config_page() {
			...
			?>
			<div class="wrap">
				<a href="http://www.escalateseo.com"><div id="yoast-icon" style="background: url(<?php echo $this->plugin_url; ?>images/ga-icon-32x32.png) no-repeat;" class="icon32"><br /></div></a>
				<h2><?php _e("WordPress Analytics Configuration") ?></h2>
				<div class="postbox-container" style="width:90%;">
					<div class="metabox-holder">	
						<div class="meta-box-sortables">
							<form action="<?php echo $this->plugin_options_url(); ?>" method="post" id="analytics-conf">
								<input type="hidden" name="plugin" value="wordpress-analytics"/>
								<?php
									...
									if ( empty($options['uastring']) && empty($options['ga_token']) && !isset($_GET['token']) ) {
										...
										if (!isset($options['ga_api_responses'][$token])) {
											...
											if (is_array($result) && $result['response']['code'] == 200) {
												...
												update_option('Escalate_Google_Analytics', $options);												
											}
										}
									...
									$desc = '<div id="position_header">The header is by far the best spot to place the tracking code. If you\'d rather place the code manually, switch to manual placement. For more info <a href="http://www.escalateseo.comwordpress/google-analytics/manual-placement/">read this page</a>.</div>';
									$desc .= '<div id="position_manual"><a href="http://www.escalateseo.comwordpress/google-analytics/manual-placement/">Follow the instructions here</a> to choose the location for your tracking code manually.</div>';
...
/*									if (class_exists('RGForms') && GFCommon::$version >= '1.3.11') {
									...
									*/
									if ( defined('WPSC_VERSION') ) {
										$pre_content = 'The WordPress e-Commerce plugin has been detected. This plugin can automatically add transaction tracking for you. To do that, <a href="http://www.escalateseo.comwordpress/google-analytics/enable-ecommerce/">enable e-commerce for your reports in Google Analytics</a> and then check the box below.';
										$rows = array();
										$rows[] = array(
											'id' => 'wpec_tracking',
...
if ( ! class_exists( 'GA_Filter' ) ) {
	class GA_Filter {
		...
		function spool_analytics() {	
			...
			$options  = get_option('Escalate_Google_Analytics');
	...
	// WordPress Analytics by Yoast v<?php echo GAWP_VERSION;  ?> | http://www.escalateseo.comwordpress/google-analytics/
	var _gaq = _gaq || [];
...
function yoast_ga_do_tracking() {
	...
	$yoast_ga_options = get_option('Escalate_Google_Analytics');
...
if (isset($analyzeit)) return false;

require_once(dirname(__FILE__) . '/analytics.class.php');

$analyzeit = new AnalyticsWP();

add_action('wp_footer', array($analyzeit, 'wp_footer'));